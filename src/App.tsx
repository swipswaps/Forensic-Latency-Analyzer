...
const MODULE_LIST = [
  "DEPS", "PSI", "CPU_CORE", "CPU_SCHED", "MEM", "NUMA", "DISK", "NET", 
...
export default function App() {
  ...
  const parseLine = (line: string) => {
    ...
    // Parse Metrics
    const metricMatch = line.match(/\[METRIC:(\w+)\]\s+([\d.]+)/);
    if (metricMatch) {
      ...
      setMetrics(prev => {
        if (key.startsWith("CPU_CORE_")) {
          ...
          return { ...prev, core_idle: { ...prev.core_idle, [core]: numVal } };
        }
        if (key === "CPU_PRESSURE") return { ...prev, cpu_pressure: numVal };
        if (key === "MEMORY_PRESSURE") return { ...prev, memory_pressure: numVal };
        if (key === "IO_PRESSURE") return { ...prev, io_pressure: numVal };
        if (key === "SELINUX_DENIALS") return { ...prev, selinux_denials: numVal };
        if (key.startsWith("DISK_") && key.endsWith("_UTIL")) return { ...prev, disk_util: numVal };
        return prev;
      });
    }
